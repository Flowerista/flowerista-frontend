...
export const cartSlice = createSlice({
	...
	reducers: {
        addCartItem: (state, {payload}: {payload: ICartItem}) => {
            const findedItem = state.cart.find(item => item.cartID === payload.cartID)
        ...
        incQuantity: (state, {payload}: {payload: PayloadId}) => {
            const itemToUpdate = state.cart.find(item => item.cartID === payload.cartID);
            if (itemToUpdate) {
        ...
        changeSize: (state, {payload}: {payload: ChangeSizePayload}) => {
            ...
            if (itemToUpdate) {
                ...
                // Создаем новую карточку и заменяем ее на место старой
                    ...
                    const newPrise = itemToUpdate.sizes.find(item => item.size === payload.size)
                    if (newPrise) {
