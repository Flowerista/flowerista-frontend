...
import open from '../../../../shared/assets/image/checkOut/open.png';
import close from '../../../../shared/assets/image/checkOut/close.png';
        ...
        <div className={styles.content} onClick={handleTabsClick}>
          {data?.map((item) => (
            <div
              key={item.id}
              onClick={() =>
                handleItemClick({ item: item, menu: name, id: item.id })
              }
              className={styles.item}
            >
              <div
                className={`${styles.square} ${selectedItems.includes(`${name}_${item.id}`) ? styles.selected : ''}`}
              ></div>
              <span>{item.name}</span>
            </div>
          ))}
