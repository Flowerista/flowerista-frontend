...
export const Search: FC<SearchProps> = ({className, type}) => {
	...
	return (
		 <form className={styles.search}>
			 <div ref={ref}
			      className={classNames(styles.searchBox, {[styles.expanded]: isExpanded}, styles[type], className)}>
								 ...
								 <li key={result.id}>
									 ...
									 <Link to={`${DataRoute.Product}/${result.id}`}>
										 ...
										 <div className={styles.searchResult__content}>
											 ...
											 <div>
												 {result.discountPrice ? <p className={styles.priceDiscount}>{result.defaultPrice} USD</p> : ''}
												 <p className={styles.defaultPrice}>{result.discountPrice ? result.discountPrice : result.defaultPrice} USD</p>
											 </div>
										 </div>
									 </Link>
								 </li>
